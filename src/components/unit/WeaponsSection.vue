<template>
  <tbody v-if="weapons">
    <template v-for="(w, wIdx) in weapons" :key="wIdx">
      <tr class="unit-details__sec">
        <td>{{!wIdx?'Weapons':''}}</td>
        <td colspan="3" class="unit-details__minititle">
          {{ w.DisplayName }}
          <span v-if="w.WeaponNumber > 0" class="sm">(&times;{{ w.WeaponNumber }})</span>
        </td>
      </tr>
...
const weaponStats = (w) => {
  ...
  if (w.BeamLifetime) stats.push({ label: 'Beam cycle', value: props.unit.beamCycle(w) })
